...
pathToKernScores.forEach(([repo, path]) => {
    ...
    getFiles(path).forEach((file) => {
        ...
        const cmd = `cat ${file} | lnnr -p | beat -cp | fb -cnl | fb -cnlr | fb -cnl --hint | degx --resolve-null -t | /Users/wolfgang/mhfreiburg/humlib-dev/bin/composite | meter -tLr | shed -s 2 -e "s/beat/beat-composite/X" | shed -s 3 -e "s/tsig/tsig-composite/X" | extractxx -I '**kern' | extractxx -I '**text' | extractxx -I '**dynam' | extractxx -I '**kern-comp' | extractxx -I '**cdata-beat' | extractxx -I '**cdata-tsig' | ridxx -LGTMId`;
        console.log(cmd);
